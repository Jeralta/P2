...
public class PercentageView extends GridPane {
	...
	public PercentageView(Pane mainView) {
		// ComboBox
			...
			for (Wijk w : wijken) {
				if (w.getWijknaam().equals(Wijknaam)) {
					for (Cijfer c : w.getCijfers()) {
						if (c instanceof Percentage) {
							Percentage p = (Percentage) c;

							oList.add(p.getOnderwerp() + " : " + p.getCijfer());
				...
				if (w.getWijknaam().equals(Wijknaam)) {
					for (Cijfer c : w.getCijfers()) {
						if (c instanceof Percentage) {
							...
							oList1.add(p.getOnderwerp() + " : " + p.getCijfer());
